...
<div class="hwdlinks">
    ...
    <ul class="hwdlinks__list"<?php echo $id; ?>>
        <?php
        foreach ($this->items as $i => &$item) {
            ...
            switch ($item->type) :
                case 'document':
                    $href = Uri::base() . $item->document;
                    $url  = "<a class='item-link item-document' href='{$href}' {$target} title='{$title}'>{$title}</a>";
                    $icon = ' <i class="fa fa-file">&nbsp;</i> ';
                    break;
                case 'link':
                    $href = $item->link;
                    $url  = "<a class='item-link item-link' href='{$href}' {$target} title='{$title}'>{$title}</a>";
                    $icon = ' <i class="fa fa-link">&nbsp;</i> ';
                    break;
                case 'menu':
                    $href = $item->menu;
                    $url  = "<a class='item-link item-menu' href='{$href}' {$target} title='{$title}'>{$title}</a>";
                    $icon = ' <i class="fa fa-link">&nbsp;</i> ';
                    break;
                case 'article':
                    $href = $item->article;
                    $url  = "<a class='item-link item-article' href='{$href}' {$target} title='{$title}'>{$title}</a>";
                    $icon = ' <i class="fa fa-link">&nbsp;</i> ';
                    break;
                default:
            ...
            if ($item->parent === true) {
                // Adding elements to accordeon menu
                echo "<input class='cd-accordion__input' type='checkbox' 
                        name ='group-{$item->id}' id='group-{$item->id}'>";
                echo "<div class='link-row link-row-lv-{$item->level}'>";
                // Adding image if is set
                if ($item->image != null) :
                    echo '<div class="item-image"><img src="' . $item->image . '"></div>';
                endif;
                echo "<label class='cd-accordion__label cd-accordion__label--icon-folder item-link-lv-{$item->level}' 
                        for='group-{$item->id}'>" .
